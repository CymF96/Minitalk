...
int	error_check(int ac, char **av)
{
	...
	if (ft_strlen(av[2]) >= 5999)
	{
		ft_printf("Error, your message has reached the 10,000 chars limit\n");
		return (1);
	}
	return (0);
}
...
int	main(int ac, char **av)
{
	...
	if (error_check(ac, av) != 0)
		exit(EXIT_FAILURE);
	server = ft_atoi(av[1]);
	size_ptr = ft_itoa(ft_strlen(av[2]));
	send_string_size(server, size_ptr);
	free(size_ptr);
	while (*av[2] != '\0')
